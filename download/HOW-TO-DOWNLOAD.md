...
不需要登录 GitHub、不需要复制直链。

- 页面按用途分了 8 组：① 客户名单 Excel → ② 老板版方案 → ③ 面谈版 → ④ 筛选清单
  → ⑤ 机制版 → ⑥ 说明与评分 → ⑦ 打包 zip → ⑧ 预览总图
- 每组卡片上直接标着文件大小，点标题即下载；预览总图可以点开先看效果再决定下不下 PDF
- 页面由 `tools/build_download_page.py` 扫描仓库生成，文件更新后重跑一次即可
  （服务本身是 `tools/serve_downloads.py`，只在会话进行时有效 —— 长期保存请下载到本机）

> 为什么不是「一律给 GitHub 链接」：GitHub 直链适合长期存档与转发，但点开是下载页、
> 还要挑文件；这份本地下载页是给"现在就要拿到文件"的人用的。两种方式都保留。
...
```text
prospects/Target-Companies.xlsx          （29 KB · 4 张表 · 28 家 × 22 列）
prospects/Target-Companies.csv           （同一份数据的通用格式）
prospects/verification.json              （逐家核实记录：官网 / 主营 / 电话 / 邮箱 / 地址 / 结论 / 来源）
```
...
| 工作表 | 里面是什么 |
| --- | --- |
| **名单** | 28 家 × **22 列**：优先级 / **优先级建议（本轮核实的结论）** / 企业名称 / 省·市 / **官网（已核实）** / **官网主营（反向验证）** / **验证结论与修正** / **联系电话** / **邮箱** / **详细地址** / **触达路径（细化到找谁、打哪个号）** / 行业细分 / 主营业务 / 适配案例 / 初筛分 / 成功面 / 失败面（待核实）/ 切入点 / 来源 / 可靠度 / 核实日期。冻结表头 + 筛选 + A4 横向排版，可直接打印 |
| **本轮核实纪要** | 核实方法（只认官网与工商公示，软文不作数）、结果统计（官网 20 家 / 电话 25 家 / 邮箱 14 家）、**12 条修正清单**（谁升档、谁降档、谁建议移出），以及打电话的三条规矩 |
...
**本轮又做了一遍「官网主营反向验证」**：逐家打开官网核对主营产品，并把触达路径细化到
具体电话 / 邮箱 / 找谁；结果 20 家拿到官网链接、25 家拿到企业电话、14 家拿到企业邮箱，
发现 12 条需要修正（例如某家原记"东莞"实为**广州花都**、某家原记"永康"主体其实在**武义**），
详见 Excel 的「本轮核实纪要」表与 `docs/08`。
...
```
download/ALL-MATERIALS.zip     约 42 MB（文件清单与实时大小见下载页）
```

含：6 份 PPTX + 6 张预览总图 + 7 份 PDF + 87 张逐页 PNG + 5 张已脱敏插图
+ 名单三件套（Excel / CSV / 核实记录 JSON）+ 9 份文档 + 7 份模板 + 3 份规格
+ 2 份评分报告 + README + 本下载指引。
